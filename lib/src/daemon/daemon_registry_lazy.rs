...
impl DaemonRegistry {
    ...
    pub fn new_with_path(db_path: PathBuf) -> Result<Self> {
        ...
        if let Some(parent) = db_path.parent() {
            if let Ok(disk_space) = fs2::available_space(parent) {
                if disk_space < 10 * 1024 * 1024 {
                    error!("Insufficient disk space at {:?}: {} bytes available", parent, disk_space);
                    return Ok(DaemonRegistry {
                        db: Arc::new(TokioMutex::new(None)),
                        memory_store,
                        is_fallback_mode: true,
                ...
                warn!("Could not check disk space at {:?}", parent);
            }
        }
        ...
        if let Ok(metadata) = fs::metadata(&db_path) {
            ...
            // Verify write permissions
            if !metadata.permissions().mode() & 0o200 != 0 {
                error!("Database directory {:?} is not writable", db_path);
                return Ok(DaemonRegistry {
                    db: Arc::new(TokioMutex::new(None)),
                    memory_store,
                    is_fallback_mode: true,
            ...
            match Self::try_open_database(&db_path) {
                ...
                Err(e) => {
                    ...
                    if attempt == max_retries {
                        // Attempt to reset corrupted database
                        if db_path.exists() {
                            if let Err(e) = fs::remove_dir_all(&db_path) {
                                error!("Failed to remove corrupted database directory {:?}: {}", db_path, e);
                            } else {
                                info!("Removed corrupted database directory {:?}", db_path);
                                if let Err(e) = fs::create_dir_all(&db_path) {
                                    error!("Failed to recreate database directory {:?}: {}", db_path, e);
                                } else {
                                    info!("Recreated clean database directory {:?}", db_path);
                                    // Try one more time after reset
                                    if let Ok(db) = Self::try_open_database(&db_path) {
                                        return Ok(DaemonRegistry {
                                            db: Arc::new(TokioMutex::new(Some(db))),
                                            memory_store,
                                            is_fallback_mode: false,
        ...
        error!("Could not open sled database after reset, using in-memory fallback with file {:?}", fallback_file);
        Ok(DaemonRegistry {
            db: Arc::new(TokioMutex::new(None)),
            memory_store,
            is_fallback_mode: true,
    ...
    fn cleanup_stale_locks(db_path: &PathBuf, lock_path: &PathBuf) -> Result<()> {
        if !lock_path.exists() {
            return Ok(());
        }
        ...
        let graphdb_processes: Vec<_> = system
            .processes()
            .values()
            .filter(|proc| proc.name().to_string_lossy().contains("graphdb"))
            .collect();

        if graphdb_processes.is_empty() {
        ...
        if let Ok(metadata) = fs::metadata(lock_path) {
            if let Ok(modified) = metadata.modified() {
                let age = SystemTime::now()
                    ...
                    .map_err(|e| anyhow!("Failed to compute lock file age: {}", e))?;
                if age > Duration::from_secs(3600) {
                    info!("Lock file {:?} is stale (age: {:?}), removing", lock_path, age);
                    fs::remove_file(lock_path)?;
                    return Ok(());
                }
            }
    ...
    fn try_open_database(db_path: &PathBuf) -> Result<Db> {
        Config::new()
            ...
            .open()
            .with_context(|| {
                let mut context = format!("Failed to open sled database at {:?}", db_path);
                if !db_path.exists() {
                    context.push_str("; path does not exist");
                } else if let Ok(metadata) = fs::metadata(db_path) {
                    if !metadata.permissions().mode() & 0o200 != 0 {
                        context.push_str("; directory is not writable");
                    }
                    if let Ok(disk_space) = fs2::available_space(db_path) {
                        context.push_str(&format!("; available disk space: {} bytes", disk_space));
                    }
                }
                context
            })
    ...
    pub async fn with_pool<R, F, Fut>(&self, f: F) -> Result<R>
    where
        F: FnOnce(Option<Db>, Arc<TokioMutex<HashMap<u16, DaemonMetadata>>>, bool) -> Fut + Send + 'static,
        Fut: std::future::Future<Output = Result<R>> + Send,
        R: Send + 'static,
    {
        let db = self.db.lock().await.as_ref().cloned();
        let memory_store = self.memory_store.clone();
        let is_fallback_mode = self.is_fallback_mode;
        tokio::task::spawn_blocking(move || {
            let rt = tokio::runtime::Handle::current();
            rt.block_on(f(db, memory_store, is_fallback_mode))
        })
        .await
        .map_err(|e| anyhow!("Thread pool task failed: {}", e))?
    }

    pub async fn with_pool_and_fallback<R, F, Fut>(&self, f: F) -> Result<R>
    where
        F: FnOnce(Option<Db>, Arc<TokioMutex<HashMap<u16, DaemonMetadata>>>, bool, Arc<TokioMutex<bool>>) -> Fut + Send + 'static,
        Fut: std::future::Future<Output = Result<R>> + Send,
        R: Send + 'static,
    {
        let db = self.db.lock().await.as_ref().cloned();
        let memory_store = self.memory_store.clone();
        let is_fallback_mode = self.is_fallback_mode;
        let fallback_loaded = self.fallback_loaded.clone();
        tokio::task::spawn_blocking(move || {
            let rt = tokio::runtime::Handle::current();
            rt.block_on(f(db, memory_store, is_fallback_mode, fallback_loaded))
        })
        .await
        .map_err(|e| anyhow!("Thread pool task failed: {}", e))?
    }
