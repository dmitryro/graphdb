...
pub async fn handle_interactive_command(
    ...
    match command {
        ...
        CommandType::Rest(rest_cmd) => {
            handlers::handle_rest_command_interactive(rest_cmd, rest_api_shutdown_tx_opt, rest_api_port_arc.clone(), rest_api_handle).await?;
        }
        ...
        CommandType::Version => {
            handlers::display_rest_api_version(rest_api_port_arc.clone()).await; // Fixed: Pass rest_api_port_arc
        }
        CommandType::Health => {
            handlers::display_rest_api_health(rest_api_port_arc.clone()).await; // Fixed: Pass rest_api_port_arc
        }
        CommandType::ReloadAll => {
            handlers::reload_all_interactive(
                daemon_handles,
                rest_api_shutdown_tx_opt,
                rest_api_port_arc.clone(), // Pass rest_api_port_arc
                rest_api_handle,
                storage_daemon_shutdown_tx_opt.clone(),
                storage_daemon_handle.clone(),
                storage_daemon_port_arc.clone(),
