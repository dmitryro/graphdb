...
// Helper function to wait for ZMQ readiness
async fn wait_for_zmq_ready(daemon: &DaemonMetadata, max_wait_secs: u64, retry_interval_ms: u64) -> Result<()> {
    const MAX_RETRIES: u32 = 5;
    let mut attempts = 0;

    while attempts < MAX_RETRIES {
        if daemon.zmq_ready {
            info!("ZMQ is ready for port {}", daemon.port);
            println!("===> ZMQ IS READY FOR PORT {}", daemon.port);
            return Ok(());
        }

        info!("ZMQ not ready for port {}. Waiting... (Attempt {}/{})", daemon.port, attempts + 1, MAX_RETRIES);
        println!("===> ZMQ NOT READY FOR PORT {}. WAITING... (Attempt {}/{})", daemon.port, attempts + 1, MAX_RETRIES);

        let registry = GLOBAL_DAEMON_REGISTRY.get().await;
        if let Ok(Some(updated_daemon)) = registry.get_daemon_metadata(daemon.port).await {
            if updated_daemon.zmq_ready {
                info!("ZMQ is now ready for port {} after waiting", daemon.port);
                println!("===> ZMQ IS NOW READY FOR PORT {} AFTER WAITING", daemon.port);
                return Ok(());
            }
        }

        tokio::time::sleep(TokioDuration::from_millis(retry_interval_ms)).await;
        attempts += 1;
    }

    Err(anyhow!("ZMQ server not ready for port {} after {} attempts", daemon.port, MAX_RETRIES))
}
...
async fn handle_kv_sled_zmq(key: String, value: Option<String>, operation: &str) -> Result<()> {
    ...
    const ZMQ_READY_TIMEOUT_SECS: u64 = 10;
    const ZMQ_READY_RETRY_INTERVAL_MS: u64 = 1000;
    ...
    wait_for_zmq_ready(daemon, ZMQ_READY_TIMEOUT_SECS, ZMQ_READY_RETRY_INTERVAL_MS).await
        .map_err(|e| anyhow!("Failed to confirm ZMQ readiness for port {}: {}", daemon.port, e))?;

    // Check if socket file exists
    if !tokio::fs::metadata(&socket_path).await.is_ok() {
        error!("IPC socket file {} does not exist despite zmq_ready=true. Daemon may not be running properly on port {}.", socket_path, daemon.port);
        return Err(anyhow!("IPC socket file {} does not exist despite zmq_ready=true. Daemon may not be running properly on port {}.", socket_path, daemon.port));
    }
